...
class crims_hitman():
    ...
    def clubs(self, *args):
        ...
        if "hp" in args:
            arg = "hp"
        self.change_logs("/nightlife", "enter", self.club, arg)
        self.kill_people()
    ...
    def change_logs(self, route, action, arguments, *args):
        ...
        Logs = Bypass.logs_bypass(self.action, self.arguments, arg)
        post_route = ""
        move = ""
        ...
        if self.action == "exit":
            ...
            if arg == "rob":
                post_route = "https://www.thecrims.com/api/v1/rob"
            else:
                post_route = "https://www.thecrims.com/api/v1/nightclub"
                move = "https://www.thecrims.com/newspaper#/nightlife/nightclub"

        #// sends the request to server
        self.browser.execute_script(js_main_requests_pattern, dumps(Logs), post_route, self.x_request, move, arg)
    ...
    def kill_people(self):
        ...
        people_to_avoid = ["Hitman","Padrino", "Godfather", "Mobster"]
        ...
        try:
            ...
            if self.percent_stamina < 50:
                ...
                if int(user_respect[0]) > 100000 and int(user_respect[0]) < 20000000 and not "nend" in user_name:
                    for avoid in people_to_avoid:
                        if avoid in user_proffesion:
                            if avoid == "Hitman":
                                if user_name in self.killers == False:
                                    break
    ...
    def robbery(self):
        self.browser.execute_script("""window.location = "https://www.thecrims.com/newspaper#/robberies" """)
        try:
            for x in range(4):
                self.change_logs("/robberies", "enter", 24, "rob")
                sleep(0.5)
            sleep(2)
            self.clubs("hp")
            exit_but = WebDriverWait(self.browser, 15).until(EC.presence_of_element_located((By.XPATH,"//div[@id='page']//div[@id='content']//table[@id='content_table']//tbody//tr//td//div[@id='content_middle']//div[@class='content_style main-content ']//div[3]//div//button[@class='btn btn-inverse btn-large pull-right']")))
            exit_but_id = exit_but.get_attribute("id")
            self.change_logs("/nightlife/nightclub", "exit", str(exit_but_id[12:]))
            for x in range(3):
                exit_but.click()
            sleep(2)
            self.robbery()
    ...
    def restore(self, exit_but, exit_id):
        self.change_logs("/nightlife", "enter", str(exit_id[12:]))
